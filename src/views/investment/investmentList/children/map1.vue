...
export default {
  ...
  destroyed () {
    this.map = null
    console.log('我销毁了==destoryed')
    // 移除覆盖物所有添加的事件
    this.overlays.forEach(item => {
      item.removeEventListener()
    })
    this.markers.forEach(item => {
      item.removeEventListener('click', this.markerFn)
    })
  },
  watch: {
    city (val, oldval) {
  ...
  methods: {
    ...
    initMap (BMap) {
      // 绘制地图
      let map = this.map = this.createMap(BMap)
      this.setCity()
      // 添加覆盖物
      this.overlays = this.addoverlays(map, BMap)
      // 添加marker点
      this.markers = this.addmarkers(map, BMap)
      this.markers.forEach(item => item.hide())
      // 添加事件
      this.bindEvent(map, BMap, this.overlays, this.markers)
    },
    createMap (BMap) {
      // 创建地图实例
    ...
    bindEvent (map, BMap, overlays, markers) {
      ...
      overlays.forEach(item => {
        item.addEventListener('touchstart', function (e) {
          ...
          that.$emit('areaChoose', this.itemData)
          return false
        })
      })

      this.markerFn = function () {
        console.log(this.itemData)
        that.$emit('projectChoose', this.itemData)
      }
      markers.forEach(marker => {
        marker.addEventListener('click', this.markerFn)
      })
    ...
    customOverlay (BMap) {
      ...
      ComplexCustomOverlay.prototype.addEventListener = function (evnetName, fn) {
        console.log('fn----------------------', fn)
        this._fn = fn.bind(this) || this._fn
        this._div.addEventListener(evnetName, fn.bind(this) || this._fn)
      }
      // 移除事件
      ComplexCustomOverlay.prototype.removeEventListener = function () {
        this._div.removeEventListener('touchstart', this._fn)
      }
