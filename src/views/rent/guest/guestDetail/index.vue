<template>
  <div class="page page_bg">
    ...
    <div class="page_bd guestDetail">
      <div class="content">
        <div class="header clearfix" @click.stop="toCustomerDetail">
          ...
          <div class="msg">
            ...
            <p class="company">
              <span class="link" v-show="detailData.CstType!='P'">联系人：{{detailData.LinkMan}}<span v-show="detailData.LinkMan">{{com_sex()}}</span></span>
            </p>
            <p class="contact">
      ...
      <div class="footer">
        <div class="btn" @click.stop="toChangeGuest">编辑</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    toChangeGuest () {
      this.$store.commit('updateGuestDetail', this.detailData)
      this.$router.push(`/changeGuest/${this.$route.params.id}`)
    },
    async getData () {
      // let obj = {
